...
describe('resolveUpstreamKey', () => {
  ...
  it('should handle null input gracefully', () => {
    const result = resolveUpstreamKey(null)
...
describe('upstream-add', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'upstream-add-test-'))
  ...
  describe('e2E: add upstream from URL', () => {
    it('should add a new upstream, discover skills, hash them, and update meta.json', async () => {
      ...
      const upstreamDir = path.join(tempDir, 'upstream', 'test-upstream')
      await fs.mkdir(upstreamDir, { recursive: true })

      // Create mock SKILL.md files (simulating what would be in the real upstream)
      const skillDir1 = path.join(upstreamDir, 'skill-one')
      await fs.mkdir(skillDir1)
      await fs.writeFile(path.join(skillDir1, 'SKILL.md'), '# Skill One\n\nDescription of skill one')

      const skillDir2 = path.join(upstreamDir, 'skill-two')
      await fs.mkdir(skillDir2)
      await fs.writeFile(path.join(skillDir2, 'SKILL.md'), '# Skill Two\n\nDescription of skill two')

      // Setup: Create initial meta.json
      const metaJsonPath = path.join(tempDir, 'meta.json')
      await fs.writeFile(
        metaJsonPath,
        JSON.stringify(
          {
      ...
      const updatedMeta = JSON.parse(
        await fs.readFile(metaJsonPath, 'utf-8'),
    ...
    it('should be idempotent: running twice with same URL updates metadata', async () => {
      ...
      const upstreamDir = path.join(tempDir, 'upstream', 'idempotent-test')
      await fs.mkdir(upstreamDir, { recursive: true })

      const skillDir = path.join(upstreamDir, 'skill-one')
      await fs.mkdir(skillDir)
      await fs.writeFile(path.join(skillDir, 'SKILL.md'), '# Skill One\n\nVersion 1')

      // Setup: Create initial meta.json
      const metaJsonPath = path.join(tempDir, 'meta.json')
      await fs.writeFile(
        metaJsonPath,
        JSON.stringify({ upstreams: {} }, null, 2),
      )
      ...
      const finalMeta = JSON.parse(await fs.readFile(metaJsonPath, 'utf-8')) as Record<string, unknown>
    ...
    it('should update selected skills when re-run with different selection', async () => {
      ...
      const upstreamDir = path.join(tempDir, 'upstream', 'update-test')
      await fs.mkdir(upstreamDir, { recursive: true })

      for (let i = 1; i <= 2; i++) {
        const skillDir = path.join(upstreamDir, `skill-${i}`)
        await fs.mkdir(skillDir)
        await fs.writeFile(path.join(skillDir, 'SKILL.md'), `# Skill ${i}`)
      }

      const metaJsonPath = path.join(tempDir, 'meta.json')
      await fs.writeFile(metaJsonPath, JSON.stringify({ upstreams: {} }, null, 2))
      ...
      const finalMetaUpdate = JSON.parse(await fs.readFile(metaJsonPath, 'utf-8')) as Record<string, unknown>
    ...
    it('should normalize different GitHub URL formats to https', async () => {
      ...
      const upstreamDir = path.join(tempDir, 'upstream', 'ssh-format')
      await fs.mkdir(upstreamDir, { recursive: true })
      ...
      const metaAfterSSH = JSON.parse(await fs.readFile(metaJsonPath, 'utf-8')) as Record<string, unknown>
    ...
    it('should compute consistent hashes for skill content', async () => {
      ...
      const upstreamDir = path.join(tempDir, 'upstream', 'hash-test')
      await fs.mkdir(upstreamDir, { recursive: true })
    ...
    it('should discover skills in nested directories', async () => {
      ...
      const upstreamDir = path.join(tempDir, 'upstream', 'nested-test')
      await fs.mkdir(upstreamDir, { recursive: true })

      // Top-level skill
      const skill1Dir = path.join(upstreamDir, 'skill-one')
      await fs.mkdir(skill1Dir)
      await fs.writeFile(path.join(skill1Dir, 'SKILL.md'), '# Skill One')

      // Nested skill
      const nestedDir = path.join(upstreamDir, 'subdir', 'skill-two')
      await fs.mkdir(nestedDir, { recursive: true })
      await fs.writeFile(path.join(nestedDir, 'SKILL.md'), '# Skill Two')
      ...
      expect(result.discoveredSkills).toHaveLength(2)
      const paths = result.discoveredSkills.map(s => s.path).sort((a, b) => a.localeCompare(b))
      expect(paths).toContain('skill-one')
      expect(paths).toContain('subdir/skill-two')

      // Assert: meta.json has both
      const meta = JSON.parse(await fs.readFile(metaJsonPath, 'utf-8')) as Record<string, unknown>
    ...
    it('should reject adding upstream with same key but different URL', async () => {
      ...
      const upstreamDir1 = path.join(tempDir, 'upstream', 'collision')
      await fs.mkdir(upstreamDir1, { recursive: true })
      await fs.mkdir(path.join(upstreamDir1, 'skill-one'), { recursive: true })
      await fs.writeFile(path.join(upstreamDir1, 'skill-one', 'SKILL.md'), '# Skill')
      ...
      const result = await Effect.runPromise(
        upstreamAdd({
          ...
          Effect.provide(SkillDiscoveryService.Default),
          Effect.provide(SkillHashService.Default),
          Effect.catchTag('UpstreamConflict', err => Effect.succeed(err)),
        ),
      )
    ...
    it('should allow same URL under multiple different upstream keys', async () => {
      ...
      const upstream1Dir = path.join(tempDir, 'upstream', 'antfu-original')
      await fs.mkdir(upstream1Dir, { recursive: true })
      await fs.mkdir(path.join(upstream1Dir, 'skill-a'), { recursive: true })
      await fs.writeFile(path.join(upstream1Dir, 'skill-a', 'SKILL.md'), '# Skill A')

      const upstream2Dir = path.join(tempDir, 'upstream', 'antfu-alias')
      await fs.mkdir(upstream2Dir, { recursive: true })
      await fs.mkdir(path.join(upstream2Dir, 'skill-a'), { recursive: true })
      await fs.writeFile(path.join(upstream2Dir, 'skill-a', 'SKILL.md'), '# Skill A')
    ...
    it('should support multiple upstreams in same meta.json', async () => {
      ...
      const upstream1Dir = path.join(tempDir, 'upstream', 'upstream-one')
      await fs.mkdir(upstream1Dir, { recursive: true })
      await fs.mkdir(path.join(upstream1Dir, 'skill-a'), { recursive: true })
      await fs.writeFile(path.join(upstream1Dir, 'skill-a', 'SKILL.md'), '# Skill A')

      const upstream2Dir = path.join(tempDir, 'upstream', 'upstream-two')
      await fs.mkdir(upstream2Dir, { recursive: true })
      await fs.mkdir(path.join(upstream2Dir, 'skill-b'), { recursive: true })
      await fs.writeFile(path.join(upstream2Dir, 'skill-b', 'SKILL.md'), '# Skill B')
  ...
  describe('e2E: prompting behavior (no upstreamKey provided)', () => {
    it('should prompt with available candidates when upstreamKey is not provided', async () => {
      // Setup: Create upstream directory
      const upstreamDir = path.join(tempDir, 'upstream', 'antfu')
      await fs.mkdir(upstreamDir, { recursive: true })

      const skillDir = path.join(upstreamDir, 'test-skill')
      await fs.mkdir(skillDir)
    ...
    it('should directly prompt for custom name when all candidates are taken', async () => {
      // Setup: Create upstream directory
      const upstreamDir = path.join(tempDir, 'upstream', 'effect')
      await fs.mkdir(upstreamDir, { recursive: true })
    ...
    it('should use selected candidate from list when user picks one', async () => {
      // Setup: Create upstream directory
      const upstreamDir = path.join(tempDir, 'upstream', 'vueuse')
      await fs.mkdir(upstreamDir, { recursive: true })
    ...
    it('should prompt for custom name when user selects "Other"', async () => {
      // Setup: Create upstream directory
      const upstreamDir = path.join(tempDir, 'upstream', 'custom-author')
      await fs.mkdir(upstreamDir, { recursive: true })
    ...
    it('should warn about taken candidates when prompting', async () => {
      // Setup: Create upstream directory
      const upstreamDir = path.join(tempDir, 'upstream', 'test-taken')
      await fs.mkdir(upstreamDir, { recursive: true })
